...
const CrabcakeAvatar = (function(){
  ...
  function buildPetSVG(state, size=40) {
    ...
    return `<svg width="${size}" height="${size}" viewBox="0 0 130 130" xmlns="http://www.w3.org/2000/svg">
      ...
      <g clip-path="url(#pc)">${svg}</g>
      <circle cx="65" cy="65" r="62" fill="none" stroke="rgba(255,255,255,0.3)" stroke-width="3"/>
    </svg>`;
  }
  ...
  function injectHUD(targetEl, options) {
    ...
    let petImg = '';
    if(options.showPet !== false && state.petId && state.petId !== 'none') {
      const petSVG = buildPetSVG(state, petSize);
